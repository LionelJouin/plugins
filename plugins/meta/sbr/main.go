...
func cmdAdd(args *skel.CmdArgs) error {
	...
	err = withLockAndNetNS(args.Netns, func(_ ns.NetNS) error {
		// If the table is set, the routes are not moved to a new table and only
		// a sbr is created. Routes can be added to that table by a previous chain element.
		if conf.Table != nil {
...
// doRoutesWithTable sets up all the rules during an add.
...
func tidyRules(iface string, table *int) error {
	...
	if table != nil {
		// Since when the table is set, only rules are created on add. Only rules
		// with the table id passed as parameter must be removed.
